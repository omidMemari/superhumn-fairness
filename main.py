...
class Super_human:
  ...
  def get_samples_demo_indexed(self):
    start_time = time.time()
    print("--- %s in get_samples_demo_indexed ---" % (time.time() - start_time))
    ...
    print("--- %s end of get_samples_demo_indexed ---" % (time.time() - start_time))
    return self.sample_matrix_demo_indexed
    

  def get_sample_loss(self, demo_index, feature_index):
  ...
  def compute_grad_theta(self):
    ...
    for j, x in enumerate(tqdm(self.demo_list)):
      ...
      for k in range(self.num_of_features):
        sample_loss = self.get_sample_loss(j, k)
    ...
    print("subdom tensor sum: ", np.sum(self.subdom_tensor))
